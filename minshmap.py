...
def map_read(seq, index, k, hfrac, theta):
    ...
    # Candidate windows: drop each rare seed's hits into overlapping buckets (b and
    # b-1) so any read-sized region falls wholly inside at least one bucket.
    ...
    best = None
    for sid, b in sorted(cand):              # score each window (sorted -> deterministic)
        lo, hi = b * W, (b + 2) * W
        used = matches = codir = 0
        r_min = r_max = -1
        for h in order:                      # add seeds rarest-first, prune early
            used += 1
            ...
            for s2, pos, hstrand in hits:
                if s2 == sid and lo <= pos < hi:
                    ...
                    r_max = max(r_max, pos)
                    break                    # a read k-mer counts in a window once
            if 1 - (used - matches) / m < theta:   # SEED HEURISTIC: window is hopeless
                break
        else:                                # ran out of seeds without pruning -> keep
            score = matches / m              # containment = fraction of read k-mers hit

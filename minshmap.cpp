...
struct Mapping { int sid, t_start, t_end; double score; int codir; bool ok = false; };
...
static Mapping map_read(const string &seq, const Index &idx, int k, int w, double theta) {  // best reference window, or ok=false
    ...
    for (long long key : windows) {
        ...
        if (score >= theta && (!best.ok || score > best.score || (score == best.score && key < best_key))) { best = {sid, ws.r_min, ws.r_max + k, score, ws.codir, true}; best_key = key; }
    }
    return best;
}
...
static void map_chunk(const vector<std::pair<string, string>> &reads, size_t lo, size_t hi, const Index &idx, int k, int w, double theta, vector<string> &out) {
    for (size_t i = lo; i < hi; ++i) {
        ...
        os << reads[i].first << '\t' << qlen << '\t' << 0 << '\t' << qlen << '\t' << (best.codir >= 0 ? '+' : '-') << '\t'
           << seg.name << '\t' << seg.length << '\t' << best.t_start << '\t' << best.t_end << '\t' << nmatch << '\t' << (best.t_end - best.t_start) << '\t' << 60 << '\n';
        out[i] = os.str();
    }
}
